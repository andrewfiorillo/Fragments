...
var img = {
	...
	tilesizes: [8, 16, 20, 40, 80, 100, 200, 400],
	
	tilesizeSlider: function() {
		
		$("#tilesize")
			.val(img.tilesizes[5])
			.prev("label")
			.children(".count")
			.text(img.tilesizes[5]);
		
		$("#tilesize_slider").slider({
			value: 4,
			min: 0,
			max: 7,
			step: 1,
			slide: function( event, ui ) {
				$("#tilesize")
					.val(img.tilesizes[ui.value])
					.prev("label")
					.children(".count")
					.text(img.tilesizes[ui.value]);
			}
		});
	},
	...
	sliders: function() {
		...
		$("#imagesize_slider").slider({
			value: $("#imagesize").val() || 400,
			min: 100,
			max: 1000,
			step: 100,
			slide: function( event, ui ) {
